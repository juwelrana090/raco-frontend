...
## 2026-07-11 Audit Follow-ups

#### [API Client] — Must support wrapped and direct responses

- **What Happens**: Some endpoints return `{ success, data }` while others return direct arrays/objects.
- **Why**: Backend controllers are not fully uniform in response envelope usage.
- **How to Avoid**: Keep `apiClient` unwrapping logic envelope-aware and fallback-safe for direct responses.
- **Discovered**: 2026-07-11

#### [API Client] — 204 No Content must bypass JSON parsing

- **What Happens**: Calling `response.json()` on 204 responses throws runtime errors.
- **Why**: 204 responses have no body.
- **How to Avoid**: Return early for `response.status === 204`.
- **Discovered**: 2026-07-11

#### [Products] — Product list shape is `{ products, pagination }`

- **What Happens**: Reading `items` or top-level `total` causes empty lists or wrong totals.
- **Why**: Backend wraps product list meta under `pagination`.
- **How to Avoid**: Always read `products` and `pagination.total/page/totalPages`.
- **Discovered**: 2026-07-11

#### [Orders/Payments] — Enums are uppercase from backend

- **What Happens**: Lowercase maps (`pending`, `paid`, `stripe`) break badge colors and filters.
- **Why**: Backend enum values are uppercase (`PENDING`, `PAID`, `STRIPE`, etc.).
- **How to Avoid**: Use uppercase for logic/filtering, lowercase only for display text.
- **Discovered**: 2026-07-11
