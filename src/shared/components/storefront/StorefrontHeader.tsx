      ...
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="h-16 flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="text-xl font-bold text-gray-900 dark:text-white">Raco</span>
          </Link>
          ...
          <div className="flex items-center gap-2">
            {/* Cart */}
            <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-xl text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2">
                <Link href="/account/dashboard" className="hidden sm:flex h-10 items-center gap-2 rounded-xl px-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                ...
                <button onClick={clearAuth} className="hidden sm:block text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 px-2">
                  Logout
                </button>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <Link href="/auth/login" className="hidden sm:block text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-brand-500 px-2">
                  Login
                </Link>
                <Link href="/auth/register" className="rounded-xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-600 transition-colors">
                  Sign up
                </Link>
              </div>
            )}
            ...
            {/* Mobile menu toggle */}
            <button onClick={() => setMobileOpen(!mobileOpen)} className="flex md:hidden h-10 w-10 items-center justify-center rounded-xl text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 pb-4">
          ...
          <div className="flex flex-col gap-1">
            {[{ href: '/', label: 'Home' }, { href: '/shop', label: 'Shop' }].map((link) => (
            ...
            {isAuthenticated && (
              <Link href="/account/dashboard" onClick={() => setMobileOpen(false)}
                className="px-3 py-2 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"
              >
                My Account
              </Link>
