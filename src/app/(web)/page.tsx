...
export default function HomePage() {
  const { data: productsData, isLoading: productsLoading } = useQuery({
    ...
    queryFn: () => storefrontApi.getProducts({ limit: 8, status: 'ACTIVE' } as any),
  });
    ...
    <div className="pb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <p className="mt-6 text-lg text-gray-300 leading-relaxed max-w-md">
                Discover thousands of quality products at great prices. Fast delivery, easy returns, and exceptional customer service.
              </p>
            ...
            <div className="hidden lg:flex items-center justify-center">
              <div className="relative">
                ...
                <div className="absolute -top-4 -right-8 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-3 text-sm">
                  ...
                  <p className="text-gray-300 text-xs">On orders above ৳2,000</p>
                </div>
                <div className="absolute -bottom-4 -left-8 rounded-2xl bg-brand-500/20 backdrop-blur-md border border-brand-500/30 p-3 text-sm">
                  ...
                  <p className="text-gray-300 text-xs">Stripe & bKash accepted</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 space-y-16 pt-16">
              ...
              {categories.slice(0, 6).map((cat: any, i: number) => {
                ...
                return (
                  <Link key={cat.id} href={`/shop?category=${cat.id}`}>
                    <div className={`group flex flex-col items-center justify-center gap-2 rounded-2xl bg-gradient-to-br ${colors[i % colors.length]} border p-5 text-center transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 cursor-pointer`}>
                      <span className="text-3xl">{getCategoryIcon(cat.name)}</span>
                      <p className="text-xs font-semibold text-gray-700 dark:text-gray-200 line-clamp-2">{cat.name}</p>
                    </div>
                  </Link>
                );
              })}
        ...
        <section>
          <div className="mb-8 flex items-end justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-brand-500 mb-1">This Week</p>
        ...
        <section className="rounded-3xl bg-gradient-to-r from-brand-500 to-brand-700 p-8 lg:p-12 text-white overflow-hidden relative">
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div>
              <p className="text-brand-200 text-sm font-semibold uppercase tracking-widest mb-2">Limited Time Offer</p>
              <h2 className="text-3xl lg:text-4xl font-extrabold leading-tight">
                Pay with bKash & Get Instant Discount
              </h2>
              <p className="mt-3 text-brand-100 text-sm leading-relaxed max-w-md">
                Use bKash mobile banking at checkout for a seamless and fast payment experience.
              </p>
            </div>
